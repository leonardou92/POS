...
import { setApiToken } from '../services/api'; // Import setApiToken
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const { clearCart, setCustomer } = useCartStore(); // Use functions from the cart store
    ...
    const handleLogin = async (username: string, password: string): Promise<void> => {
        setIsLoading(true);
        try {
            // Reset all user-related variables in localStorage before setting new values
            localStorage.removeItem('authToken');
            localStorage.removeItem('user');
            ...
            if (response.status) {
                ...
                setApiToken(response.token); // Set api token
    ...
    const logout = () => {
        // Clear only authToken and user from localStorage
        localStorage.removeItem('authToken');
        localStorage.removeItem('user');
        setApiToken(null);
        ...
        clearCart();
        setCustomer({  // Reset customer data
            razonSocial: '',
            registroFiscal: '',
            direccionFiscal: '',
            email: '',
